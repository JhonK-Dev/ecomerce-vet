...
export default function HistoriasClinicasPage() {
  ...
  const loadUserData = async (role: string) => {
    try {
      setLoading(true)
      ...
      if (role === 'client') {
        ...
        if (userPets.length > 0) {
          ...
          const petStats = await MedicalRecordService.getMedicalStats(userPets[0].id)
          setStats(petStats)
        }
                    ...
                    {pets.map(pet => (
                      <button
                        ...
                        className={`w-full p-4 text-left hover:bg-muted transition-colors ${
                          selectedPet?.id === pet.id ? 'bg-muted border-r-2 border-primary' : ''
                        }`}
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-sm text-muted-foreground">
                              {MedicalRecordService.getSpeciesLabel(pet.species)} • {MedicalRecordService.formatAge(pet.birthDate)}
                            </div>
                          </div>
                        </div>
                      </button>
                ...
                <Tabs defaultValue="profile" className="space-y-6">
                  ...
                  <TabsContent value="profile">
                    <PetProfile
                      ...
                      showOwnerInfo={userRole === 'veterinarian'}
                    />
                  </TabsContent>
                  ...
                  <TabsContent value="records">
                    <MedicalRecordsList
                      ...
                      onAddRecord={handleAddRecord}
                      onEditRecord={(record) => console.log('Editar registro', record)}
                      showAddButton={userRole === 'veterinarian'}
                    />
                  </TabsContent>
                  ...
                  <TabsContent value="alerts">
                    <div className="space-y-6">
                      ...
                      <MedicalAlerts
                        petId={selectedPet.id}
                        showCompleted={false}
                        onAlertComplete={(alert) => console.log('Alerta completada', alert)}
                      />
                    </div>
                  </TabsContent>
                </Tabs>
                ...
                <Card>
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="text-muted-foreground">
                      Elige una mascota de la lista para ver su información médica.
                    </p>
                  </CardContent>
                </Card>
              )}
